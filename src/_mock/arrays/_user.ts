...
export const _jobList = [...Array(24)].map((_, index) => ({
  ...
  datePosted: '12-05-2023',
  applications: randomInArray([100, 55, 123, 500, 200]),
  status: randomInArray(['ongigng', 'apploval pending', 'innactive']),
}));

export const _billingList = [...Array(24)].map((_, index) => ({
  id: _mock.id(index),
  date: '12-05-2023',
  amount: _mock.name.fullName(index),
  attachment: _mock.email(index),
  invoice: _mock.boolean(index),
  approvedBy: _mock.boolean(index),
  status: randomInArray(['active', 'banned']),
}));
