...
const Ollcode = forwardRef<HTMLDivElement, LogoProps>(
  ({ disabledLink = false, sx, ...other }, ref) => {
    ...
    return <Link sx={{ display: 'contents' }}>{ollcode}</Link>;
  }
);
    ...
    const miniLogo = (
      <Box
        ...
        sx={{
          ...
          height: "auto",
          display: 'inline-flex',
          alignSelf:{lg:"center", md:"left", sm:"left", xs:"left"},
          ...sx,
        }}
        {...other}
      >
        <img src={PRSMiniLogo} alt=''/>
      </Box>
    );
...
const LoginText = forwardRef<HTMLDivElement, LogoProps>(
  ({ disabledLink = false, sx, ...other }, ref) => {
    const loginText = (
      <Box
        component="div"
        sx={{
          width: { lg: 410, md: 408, sm: 400, xs: '80%' },
          display: 'flex',
          flexDirection: 'row',
    ...
    return <Link sx={{ display: 'contents' }}>{loginText}</Link>;
  }
);

export { Logo, MiniLogo, Ollcode, LoginText };
