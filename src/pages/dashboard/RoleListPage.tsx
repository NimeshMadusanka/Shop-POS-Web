...
const TABLE_HEAD = [
  ...
  { id: 'monthlySalary', label: 'Monthly Salary', align: 'left' },
  { id: 'allowances', label: 'Allowances', align: 'left' },
  { id: 'deductions', label: 'Deductions', align: 'left' },
...
export default function PaymentListPage() {
  ...
  const handleEditRow = (id: string, row: any) => {
    navigate(PATH_DASHBOARD.role.edit(paramCase(id)), row);
  };

  const handleDeleteRow = (id: string) => {
    const deleteRow = tableData.filter((row: NewRoleCreate) => row?._id !== id);
  ...
  return (
    <>
      <Helmet>
        <title> Roles: List | POS system </title>
      </Helmet>
  ...
  if (filterName) {
    inputData = inputData.filter(
      (payment) =>
        payment &&
        payment.roleName &&
        payment.roleName.toLowerCase().indexOf(filterName.toLowerCase()) !== -1
    );
  }
